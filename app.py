...
def _build_pdf(business_name: str, plan_text: str, logo_bytes: bytes | None = None) -> BytesIO:
    buf = BytesIO()
    ...
    # Optional cover page with logo
    if logo_bytes:
        try:
            cover_title = ParagraphStyle(
                "BNCoverTitle", parent=styles["Title"], fontName="Helvetica-Bold",
                fontSize=34, leading=40, textColor=GREEN, alignment=TA_CENTER, spaceAfter=10,
            )
            cover_sub = ParagraphStyle(
                "BNCoverSub", parent=styles["Normal"], fontName="Helvetica",
                fontSize=13, textColor=INK_60, alignment=TA_CENTER, spaceAfter=24,
            )
            cover_foot = ParagraphStyle(
                "BNCoverFoot", parent=styles["Normal"], fontName="Helvetica-Oblique",
                fontSize=10, textColor=INK_60, alignment=TA_CENTER,
            )
            ...
            disp_h = ih * ratio
            img_buf.seek(0)
            logo_img = Image(img_buf, width=disp_w, height=disp_h)
            logo_img.hAlign = "CENTER"

            story.append(Spacer(1, 3.5 * cm))
            story.append(logo_img)
            story.append(Spacer(1, 1.2 * cm))
            story.append(Paragraph(business_name, cover_title))
            story.append(HRFlowable(width="40%", thickness=1.2, color=GOLD,
                                    spaceBefore=4, spaceAfter=14, hAlign="CENTER"))
            story.append(Paragraph("Business Plan", cover_sub))
            story.append(Spacer(1, 4 * cm))
            story.append(Paragraph(
                f"Prepared with BizNova · {datetime.now():%B %Y}", cover_foot))
            story.append(PageBreak())
        except Exception:
            # If image fails, fall through to standard header
            pass
...
@login_required
def export_bundle():
    """Export a chosen plan with a chosen logo as a single PDF (logo on cover page)."""
    body = request.get_json(silent=True) or {}
    ...
    with get_db() as conn:
        plan_row = conn.execute(
            "SELECT id, name, content FROM plans WHERE id=? AND user_id=?",
            (plan_id, session["user_id"]),
        ).fetchone()
        if not plan_row:
            return jsonify({"error": "Plan not found."}), 404
        ...
        logo_bytes = None
        if logo_id is not None and logo_id != "":
            try:
                logo_id_int = int(logo_id)
            except (TypeError, ValueError):
                return jsonify({"error": "logo_id must be an integer"}), 400
            logo_row = conn.execute(
                "SELECT image_b64 FROM logos WHERE id=? AND user_id=?",
                (logo_id_int, session["user_id"]),
            ).fetchone()
            if not logo_row:
                return jsonify({"error": "Logo not found."}), 404
            try:
                import base64
                logo_bytes = base64.b64decode(logo_row["image_b64"])
            except Exception:
                logo_bytes = None

    name = plan_row["name"] or "BusinessPlan"
    plan_text = plan_row["content"] or ""
    try:
        pdf = _build_pdf(name, plan_text, logo_bytes=logo_bytes)
    except Exception as e:
        return jsonify({"error": f"PDF generation failed: {e}"}), 500
    return send_file(
        pdf,
